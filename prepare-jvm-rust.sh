...
TARGETS=(
    ...
    "x86_64-pc-windows-msvc       windows-x86-64     ratex_ffi.dll"
)
...
build_host() {
    ...
    cargo build --manifest-path "$RATEX_ROOT/Cargo.toml" --release -p ratex-ffi

    for entry in "${TARGETS[@]}"; do
        read -r target jna_dir lib_file <<< "$entry"
        if [ "$target" = "$host_target" ]; then
            local src="$RATEX_ROOT/target/release/$lib_file"
            local dest="$NATIVE_DIR/$jna_dir"
            if [ -f "$src" ]; then
                mkdir -p "$dest"
                cp "$src" "$dest/"
...
build_all() {
    ...
    echo "==> Building ratex-ffi for all JVM desktop platforms (parallel)..."
    local pids=()
    for entry in "${TARGETS[@]}"; do
        read -r target jna_dir lib_file <<< "$entry"
